...
Перед первым запуском необходимо запустить (до запуска файла telebot.py) файлы: fetch_nasa.py и fetch_spacex.py, чтобы Ваша коллекция фотографий космоса (папка images) наполнилась фотографиями SPACEX и NASA, которые и будут использованы ботом для постинга в телеграмм-канал. После этого убедитесь, что папка images создалась и наполнена фотграфиями и запускайте проект.
